...
@pytest.mark.flaky #Flaky test - sometimes work, sometimes don't. This line ignores them if they fail -> 'xfail'

#Need a set of tests for each function
def test_quenching_funct():
    ...
    from skypy.halo.sham import quenching_funct
...
def test_find_min():
    ...
    from skypy.halo.sham import find_min
    ...
    assert type(min_mass) == numpy.float64
    ...
    assert find_min(m_star, phi_star, alpha, cosmology, z_range, skyarea, max_mass, no_halo_min, run_anyway=True) == approx(10**(7))
    assert find_min(m_star, phi_star, alpha, cosmology, z_range, skyarea, max_mass, no_halo_max, run_anyway=True) == approx(10**(7))
    ...
    with pytest.raises(Exception) as excinfo:
        ...
        no_halo_y, no_halo_min, no_halo_max = 10000, 600, 50000
        find_min(m_star, phi_star, alpha, cosmology, z_range, skyarea, max_mass, no_halo_y)
        assert str(excinfo.value) == 'The Schechter function for galaxies is defined such that alpha should be negative, set_alpha = -alpha'
...
def test_run_file():
    ...
    from astropy import units as u
    from astropy.cosmology import FlatLambdaCDM
    from skypy.halo.sham import run_file
    ...
    test_cat, test_z = run_file('test_gal.yaml', 'galaxy', 'sm', 'z')
    ...
    assert trap(dn, z) == approx(len(test_cat), rel=np.sqrt(trap(dn, z))) #Length same within Poisson sampling error
    assert len(test_cat) == len(test_z)

    #Check errors trigger
    with pytest.raises(Exception) as excinfo:
        run_file(test_gal, 'galaxy', 'sm', 'z')
        assert str(excinfo.value) == 'File name must be a string'

    with pytest.raises(Exception) as excinfo:
        run_file('DoesNotExist.yaml', 'galaxy', 'sm', 'z')
...
def test_gen_sub_cat():
    ...
    with pytest.raises(Exception) as excinfo:
        ...
        parent_halo = colossus_mass_sampler(redshift=0.1, model='sheth99',
                                            mdef='fof', m_min=m_min, m_max=m_max,
                                            cosmology=FlatLambdaCDM, sigma8=0.8, ns=1.,
                                            size=size, resolution=1000)
        z_halo = 0.1
        gen_sub_cat(parent_halo, z_halo, alpha, beta, gamma, x)
    ...
    with pytest.raises(Exception) as excinfo:
        ...
        z_halo = [0.05, 0.1, 0.01]
        gen_sub_cat(parent_halo, z_halo, alpha, beta, gamma, x)
        assert str(excinfo.value) == 'The subhalo mass function is defined such that alpha should be positive, set_alpha = -alpha'
...
def test_gal_cat():
    ...
    #Function
    cat = galaxy_cat(m_star, phi_star, alpha, cosmology, z_range, skyarea, min_mass, max_mass, file_name)
    ...
    exp_struct = line1 + line2 + line3 + line4 + line5 + line6 + line7 + line8 + line9 + line10 + function
...
def test_assignment():
    ...
    rc_cat = schechter_smf(z_range, m_star1, phi_star1, alpha1, m_min1, m_max1, skyarea, cosmo, noise=False)[1]
    rs_cat = schechter_smf(z_range, m_star2, phi_star2, alpha2, m_min2, m_max2, skyarea, cosmo, noise=False)[1]
    bc_cat = schechter_smf(z_range, m_star3, phi_star3, alpha3, m_min3, m_max1, skyarea, cosmo, noise=False)[1]
    bs_cat = schechter_smf(z_range, m_star4, phi_star4, alpha4, m_min4, m_max2, skyarea, cosmo, noise=False)[1]
    ...
    #Function
    hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_order, rc_order, rs_order, bc_order, bs_order, qu_hs, id_hs, z_hs, print_out=False)
    
    #Check arrays shape/type
    assert hs_fin.shape == gal_fin.shape == id_fin.shape == z_fin.shape == gal_type_fin.shape
    assert hs_fin.ndim == 1

    #Check arrays contain expected values
    assert ((gal_type_fin==1) | (gal_type_fin==2) | (gal_type_fin==3) | (gal_type_fin==4)).all() #Check only these values
    assert len(np.where(gal_type_fin == 1)[0]) > 1 #Check some of each were assigned
    assert len(np.where(gal_type_fin == 2)[0]) > 1
    assert len(np.where(gal_type_fin == 3)[0]) > 1
    assert len(np.where(gal_type_fin == 4)[0]) > 1
    ...
    check_parent_t = gal_type_fin[np.where(id_fin < 0)] #Finished arrays
    check_parent_m = gal_fin[np.where(id_fin < 0)]
    check_subhal_t = gal_type_fin[np.where(id_fin > 0)]
    check_subhal_m = gal_fin[np.where(id_fin > 0)]
    check_blu_t = gal_type_fin[np.where(qu_hs == 0)]
    check_blu_m = gal_fin[np.where(qu_hs == 0)]
    check_red_t = gal_type_fin[np.where(qu_hs == 1)]
    ...
    with pytest.raises(Exception) as excinfo:
        hs_test = -1*hs_order
        hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_test, rc_order, rs_order, bc_order, bs_order, qu_hs, id_hs, z_hs)
        assert str(excinfo.value) == 'Halo masses must be positive and non-negative'
    
    with pytest.raises(Exception) as excinfo:
        rs_test = -1*rs_order
        hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_order, rc_order, rs_test, bc_order, bs_order, qu_hs, id_hs, z_hs)
        assert str(excinfo.value) == 'Galaxy masses must be positive and non-negative'
    ...
    with pytest.raises(Exception) as excinfo:
        ...
        hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_order, rc_test, rs_order, bc_order, bs_order, qu_hs, id_hs, z_hs)
        assert str(excinfo.value) == 'Red central galaxy masses were in the wrong order and have been corrected'
    ...
    with pytest.raises(Exception) as excinfo:
        ...
        hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_order, rc_order, rs_test, bc_order, bs_order, qu_hs, id_hs, z_hs)
        assert str(excinfo.value) == 'Red satellite galaxy masses were in the wrong order and have been corrected'
    ...
    with pytest.raises(Exception) as excinfo:
        ...
        hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_order, rc_order, rs_order, bc_test, bs_order, qu_hs, id_hs, z_hs)
        assert str(excinfo.value) == 'Blue central galaxy masses were in the wrong order and have been corrected'
    ...
    with pytest.raises(Exception) as excinfo:
        ...
        hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_order, rc_order, rs_order, bc_order, bs_test, qu_hs, id_hs, z_hs)
        assert str(excinfo.value) == 'Blue satellite galaxy masses were in the wrong order and have been corrected'
    ...
    with pytest.raises(Exception) as excinfo:
        id_test = np.geomspace(min(id_order), max(id_order), 10)
        hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_test, rc_order, rs_order, bc_order, bs_order, qu_hs, id_test, z_hs)
        assert str(excinfo.value) == 'All arrays pertaining to halos must be the same shape'

    with pytest.raises(Exception) as excinfo:
        z_test = np.geomspace(min(z_order), max(z_order), 10)
        hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_order, rc_order, rs_order, bc_order, bs_order, qu_hs, id_order, z_test)
        assert str(excinfo.value) == 'All arrays pertaining to halos must be the same shape'

def test_sham_plots():
    #Generate the catalogues
    from astropy.cosmology import WMAP9 #Cannot use FlatlambdaCDM
    from colossus.cosmology.cosmology import fromAstropy
    from colossus.lss import mass_function
    from astropy import units as u
    from skypy.halos.mass import colossus_mass_sampler
    ...
    #Assign the galaxies
    hs_fin, gal_fin, id_fin, z_fin, gal_type_fin = assignment(hs_order, rc_order, rs_order, bc_order, bs_order, qu_hs, id_hs, z_hs)

    #Function
    sham_rc, sham_rs, sham_bc, sham_bs, sham_cen, sham_sub = sham_plots(hs_fin, gal_fin, gal_type_fin)

    #Arrays correct type and size
    assert type(sham_rc) == type(sham_rs) == type(sham_bc) == type(sham_bs) == np.ndarray
    assert sham_rc.ndim == sham_rs.ndim == sham_bc.ndim == sham_bs.ndim == sham_cen.ndim == sham_sub.ndim == 2
    ...
    with pytest.raises(Exception) as excinfo:
        hs_test = np.linspace(min(hs_order), max(hs_order), 10)
        SHAM_arrays(hs_test, gal_fin, gal_type_fin, x)
        assert str(excinfo.value) == 'All arrays must be the same shape'
    
    with pytest.raises(Exception) as excinfo:
        gal_type_test = np.random.randint(1,5,10)
        SHAM_arrays(hs_fin, gal_fin, gal_type_test, x)
        assert str(excinfo.value) == 'All arrays must be the same shape'
    ...
    with pytest.raises(Exception) as excinfo:
        gal_test = -1*(gal_fin)
        SHAM_arrays(hs_fin, gal_test, gal_type_fin, x)
        assert str(excinfo.value) == 'Galaxy masses must be positive and non-negative'
...
def test_run_sham():
    ...
    #Function
    sham = run_sham(h_file, gal_param, cosmology, z_range, skyarea, qu_h, qu_s, sub_param = [1.91, 0.39, 0.1, 3],
                    gal_max_h = 10**(14), gal_max_s = 10**(13), print_out=False, run_anyway=True)
    ...
    with pytest.raises(Exception) as excinfo:
        h_f = 3
        run_sham(h_f, gal_param, cosmology, z_range, skyarea, qu_h, qu_s, sub_param = [1.91, 0.39, 0.1, 3],
                 gal_max_h = 10**(14), gal_max_s = 10**(13), print_out=False, run_anyway=True)
        assert str(excinfo.value) == 'Halo YAML file must be provided as a string'

    with pytest.raises(Exception) as excinfo:
        gal_p = [[1,2,3], [4,5,6]]
        run_sham(h_file, gal_p, cosmology, z_range, skyarea, qu_h, qu_s, sub_param = [1.91, 0.39, 0.1, 3],
                 gal_max_h = 10**(14), gal_max_s = 10**(13), print_out=False, run_anyway=True)
        assert str(excinfo.value) == 'The wrong number of galaxies have been provided in their parameters'
    ...
    with pytest.raises(Exception) as excinfo:
        qu_ht = [1]
        run_sham(h_file, gal_param, cosmology, z_range, skyarea, qu_ht, qu_s, sub_param = [1.91, 0.39, 0.1, 3],
                 gal_max_h = 10**(14), gal_max_s = 10**(13), print_out=False, run_anyway=True)
        assert str(excinfo.value) == 'Provided incorrect number of halo quenching parameters'

    with pytest.raises(Exception) as excinfo:
        qu_st = [1]
        run_sham(h_file, gal_param, cosmology, z_range, skyarea, qu_h, qu_st, sub_param = [1.91, 0.39, 0.1, 3],
                 gal_max_h = 10**(14), gal_max_s = 10**(13), print_out=False, run_anyway=True)
        assert str(excinfo.value) == 'Provided incorrect number of halo quenching parameters'
